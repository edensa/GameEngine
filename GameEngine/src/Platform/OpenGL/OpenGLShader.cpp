...
namespace engine
{
	namespace utils {
		...
		static shaderc_shader_kind GLShaderStageToShaderC(GLenum stage)
		{
			...
			ENGINE_CORE_ASSERT(false, "Unknown shader stage!");
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			std::string shaderFileExt = utils::GLShaderStageCachedVulkanFileExtension(stage);
			std::filesystem::path cachedPath = cacheDirectory / (shaderFilePath.filename().string() + shaderFileExt);
			...
			else
			{
				...
				if (module.GetCompilationStatus() != shaderc_compilation_status_success)
				{
					...
					ENGINE_CORE_ASSERT(false, "spirv compilation failed");
				}
	...
	void OpenGLShader::CompileOrGetOpenGLBinaries()
	{
		...
		{
			std::filesystem::path shaderFilePath = m_FilePath;
			std::string shaderFileExt = utils::GLShaderStageCachedOpenGLFileExtension(stage);
			std::filesystem::path cachedPath = cacheDirectory / (shaderFilePath.filename().string() + shaderFileExt);
